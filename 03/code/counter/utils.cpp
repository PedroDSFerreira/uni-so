...
#define check_alloc(pnt)                                                       \
  if ((pnt) == NULL)                                                           \
    do {                                                                       \
      fprintf(stderr,                                                          \
              "%s, pointer: \"%s\", function: \"%s\":%d, file: \"%s\"\n",      \
...
int string_ends_with(char *text, char *suffix) {
  ...
  return strlen(text) >= strlen(suffix) &&
         (strcmp(text + strlen(text) - strlen(suffix), suffix) == 0);
}
...
int string_find(char *text, char *substring, int count) {
  require(text != NULL, "string argument required");
  require(substring != NULL && strlen(substring) > 0,
          substring == NULL ? "string argument required" : "empty substring");
  require(count > 0,
          concat_3str("invalid number of occurrences (", int2str(count), ")"));
...
char *string_concat(char *res, int max_length, char *text, ...) {
  ...
  require(res == NULL ||
              (max_length > 0 && length_vargs_string_list(text) <= max_length),
          concat_5str("text arguments length (",
                      int2str(length_vargs_string_list(text)),
                      ") exceeds maximum length of result argument (",
                      int2str(max_length), ")"));
...
int random_boolean(int trueProb) {
  require(trueProb >= 0 && trueProb <= 100,
          concat_3str("invalid percentage (", int2str(trueProb), ")"));
...
int random_int(int min, int max) {
  require(max >= min, concat_5str("invalid interval [", int2str(min), ",",
                                  int2str(max), "]"));
...
void move_cursor(int line, int column) {
  require(line >= 0 && column >= 0,
          concat_4str("invalid cursor position, line:", int2str(line),
                      ", column:", int2str(column)));
...
char **string_list_clone(char **list) {
  ...
  char **res =
      (char **)mem_alloc((string_list_length(list) + 1) * sizeof(char *));
  int i;
...
char *int2nstring(char *res, int num, int len) {
  require(len >= numDigits(num),
          concat_7str("length argument (", int2str(len),
                      ") lower than number (", int2str(num), ") # of digits (",
                      int2str(numDigits(num)), ")"));
...
char *percentage2string(char *res, int percentage) {
  require(percentage >= 0 && percentage <= 100,
          concat_3str("invalid percentage (", int2str(percentage), ")"));
