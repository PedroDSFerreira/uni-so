 ...
 * A busy waiting delay is inserted between the insertion/retrieval of the 2
 * integers in the fifo.
 */
...
/*
 * Each producer will produce values from 1 to ni and will insert them into the
 * fifo. Note that the same value is placed into fields v1 and v2 of a fifo
 * item.
 */
 ...
 * So, each consumer works in an infinite loop, retrieving items and printing
 * them. Note that if, in an item retrieved, values v1 and v2 are different,
 * which should not happen, the printing is done in red. Otherwise it appears in
 * green.
 */
