...
bool fifoIsFull(FIFO *f) { return f->full; }

bool fifoIsEmpty(FIFO *f) { return !f->full && (f->in == f->out); }
...
void fifoInsert(FIFO *f, uint32_t id, uint32_t v1, uint32_t v2) {
  ...
  f->data[f->in].id = id;
  f->data[f->in].v1 = v1;
  bwDelay(f->dummyDelay); // to enhance the probability of occurrence of race
                          // conditions
  f->data[f->in].v2 = v2;
  f->in = (f->in + 1) % f->size;
  f->full = (f->in == f->out);
